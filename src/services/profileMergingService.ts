...
export class ProfileMergingService {
  ...
  static async getMergedProfile(admin: AdminUser): Promise<MergedProfile> {
    try {
      // Look for team member with same email (include inactive profiles for admin's own view)
  ...
  static async createOrUpdateTeamProfile(
    ...
    try {
      ...
      if (existingTeamMember) {
        // Update existing team member - keep existing is_active status
        // This preserves whether a super admin has approved them or not
        ...
        // Create new team member - set is_active to false by default
        // Only super admins can make profiles public via Team Management
        const { error } = await supabase
          .from('team_members')
          .insert({
            name: teamData.name,
            ...
            is_active: false, // Not public until super admin approves
